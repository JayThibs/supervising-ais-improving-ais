...
from utils.resource_management import ResourceManagement
...
class EvaluatorPipeline:
    ...
    def embed_responses(self, query_results_per_model):
        ...
        file_loaded, joint_embeddings_all_llms = self.data_prep.load_pkl_or_not(
    ...
    def run_clustering(self, combined_embeddings):
        file_loaded, chosen_clustering = self.data_prep.load_pkl_or_not(
    ...
    def analyze_response_embeddings_clusters(
        ...
        file_loaded, rows = self.data_prep.load_pkl_or_not(
            "rows.pkl",
            self.pickle_dir,
    ...
    def load_or_generate_approvals_data(self, approvals_type, reuse_approvals):
        ...
        file_loaded, approvals_statements_and_embeddings = self.data_prep.load_pkl_or_not(
            pickle_filename,
            self.pickle_dir,
            reuse_approvals,
        )
        ...
        if not file_loaded:
            ...
            conditions_loaded, approval_results_per_model = self.data_prep.load_pkl_or_not(
    ...
    def perform_hierarchical_clustering(
        ...
        file_loaded, hierarchy_data = self.data_prep.load_pkl_or_not(
    ...
    def visualize_loaded_data(
        ...
        dim_reduce_tsne = self.dim_reduce_obj.tsne_reduction(
            combined_embeddings,
    ...
    def run_evaluations(self):
        ...
        if has_local_models:
            gpu_availability = ResourceManagement.check_gpu_availability()
            if gpu_availability == "multiple_gpus":
                ...
                for model_batch in model_batches:
                    self.run_pipeline_for_models(
                        model_batch, gpu_availability=gpu_availability
                    )
            else:
                # Run local models sequentially
                for model_name, local_model in self.local_models.items():
                    if ResourceManagement.check_gpu_memory([(model_name, local_model)], buffer_factor=1.2):
                        local_model.load()
                        self.run_pipeline_for_models(
                            [(model_name, local_model)], gpu_availability=gpu_availability
                        )
                        local_model.unload()
                    else:
                        print(

...
class ClusteringAlgorithms:
    @staticmethod
    def run_clustering(embeddings: np.ndarray, algorithm: str, n_clusters: Optional[int], **kwargs) -> object:
        algorithm_map = {
        ...
        return algorithm_class(**kwargs).fit(embeddings)
...
class Clustering:
    ...
    def cluster_embeddings(
        ...
        clustering_algorithm: str = "SpectralClustering",
        n_clusters: Optional[int] = None,
        multiple: bool = False,
        **kwargs
    ) -> Union[Dict[str, object], object]:
        """
        Perform clustering on the given embeddings.
        ...
        Args:
            ...
            multiple: Whether to run multiple clustering algorithms.
            **kwargs: Additional arguments for the clustering algorithm.

        Returns:
            Clustering results (single algorithm or dictionary of results for multiple algorithms).
        """
        n_clusters = n_clusters or self._get_n_clusters()
        
        if multiple:
            return self._run_multiple_clustering(embeddings, **kwargs)
        else:
    ...
    def _run_multiple_clustering(self, embeddings: np.ndarray, n_clusters: int, **kwargs) -> Dict[str, object]:
        algorithms = ["OPTICS", "SpectralClustering", "AgglomerativeClustering", "KMeans"]
        return {alg: ClusteringAlgorithms.run_clustering(embeddings, alg, n_clusters, **kwargs) for alg in algorithms}

    def _get_n_clusters(self) -> int:
        return self.settings.n_clusters if not self.settings.test_mode else 10
    ...
    def _run_single_clustering(
        ...
        **kwargs
        ...
        if clustering_algorithm not in algorithm_map:
            raise ValueError(f"Unsupported clustering algorithm: {clustering_algorithm}")
        
        algorithm_class = algorithm_map[clustering_algorithm]
        n_clusters = n_clusters or self._get_n_clusters()

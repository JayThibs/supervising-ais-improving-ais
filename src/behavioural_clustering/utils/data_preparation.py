...
class DataHandler:
    ...
    def save_pickles(
        self,
        data_file_paths: Dict[str, str],  # {data_type: file_path}
        data: List[Any],
    ):
        for i, (key, path) in enumerate(data_file_paths.items()):
            if i < len(data):
                self.save_data(data[i], path)
                print(f"Saved data for {key} to {path}.")
            else:
                print(f"Warning: No data found for {key}.")

    def save_run_metadata_to_yaml(self, run_id: str, metadata: Dict[str, Any]):
        metadata_file = self.results_dir / "metadata_for_runs.yaml"
        lock = Lock()
        with lock:
            if metadata_file.exists():
                with open(metadata_file, "r") as file:
            ...
            # Convert PosixPath objects to strings in the new metadata
            serializable_metadata = self._make_serializable(metadata)

            # Update the existing metadata with the new serializable metadata
            existing_metadata[run_id] = serializable_metadata

            # Convert the entire existing_metadata to ensure all entries are serializable
            fully_serializable_metadata = self._make_serializable(existing_metadata)

            with open(metadata_file, "w") as file:
                yaml.safe_dump(fully_serializable_metadata, file)
    ...
    def get_or_create_data_file_path(
        ...
        if data_type in data_file_mapping:
            return Path(data_file_mapping[data_type])
        else:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            file_path = str(data_file_dir / f"{filename}_{timestamp}.pkl")
            data_file_mapping[data_type] = file_path

...
def validated_assistant_discriminative_compare(
    ...
    Returns:
        Dict[str, Any]: A dictionary containing the validation results with the following keys:
            - 'hypothesis_accuracies': List[float]
                Mean accuracy scores for each hypothesis.
            - 'hypothesis_p_values': List[float] 
                P-values for each hypothesis.
            - 'reasonings': Optional[List[List[str]]]
                If explain_reasoning is True, contains reasoning explanations for each prediction.
                If explain_reasoning is False, this key will not be present in the dictionary.
    """
    # Input validation
    ...
    for round_idx in range(num_rounds):
        ...
        for test_case in active_test_cases:
            if round_idx == 0:
                prompt = (
                    "You will be interacting with one of two language models. "
                    "Here is a hypothesis about how these models differ:\n"
                    f'"{test_case["description"]}"\n\n'
                    f"You can make {num_rounds} queries to the model to determine whether you're talking to Model 1 or Model 2. "
                    ...
                    "\n\nKeep in mind that the hypothesis represents a difference in the models' generative tendencies. "
                    "As such, your query should reveal which patterns the current model is more likely to exhibit. "
                    "E.g., if the hypothesis is that Model 1 talks about cats whereas Model 2 talks about dogs, "
                    "don't just ask about cats directly, but instead generate a leading question for which either "
                    "cat or dog would be a good answer. E.g., instead of 'What do you think about cats?', try "
                    "'Describe an interesting habit of either cats or dogs.'"

                    "\n\nWrite your first query to the model:"
                )

            else:
                # Build full interaction history
                interaction_history = '\n\n'.join([
                    f'Round {i+1}:\n{q}{r}' 
                ...
                prompt = (
                    "You will be interacting with one of two language models.\n"
                    "Here is the hypothesis about how these models differ:\n"
                    f'"{test_case["description"]}"\n\n'
        ...
        try:
            ...
            for test_case, query in zip(active_test_cases, queries):
                test_case['assistant_messages'].append("Input: " + query + "\nResponse:")
        except Exception as e:
    ...
    if explain_reasoning:
        return_dict["reasonings"] = all_reasonings

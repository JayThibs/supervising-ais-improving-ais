...
def generate_responses_in_batches(
    ...
    try:
        for i in range(0, len(queries), batch_size):
            ...
            batch_responses = [tokenizer.decode(output, skip_special_tokens=True) 
...
# Currently in development as a better / faster version of validated_assistant_discriminative_compare:
def validated_assistant_discriminative_compare(
...
) -> Union[Tuple[List[float], List[float]], Tuple[List[float], List[float], List[List[str]]]]:
    """
    ...
    Args:
        ...
        logger (Optional[BoundLoggerLazyProxy]): The logger to use for logging API requests and responses.
        num_workers (int): Number of workers to use for parallel API requests.
        model_batch_size (int): Number of model responses to generate at once.
    Returns:
    ...
    # Initialize tracking
    all_predictions_accurate = [[] for _ in range(len(difference_descriptions))]
    all_reasonings = [[] for _ in range(len(difference_descriptions))] if explain_reasoning else None
    ...
    for desc_idx, description in enumerate(difference_descriptions):
        for run_idx in range(2 * num_validation_runs):
            ...
            test_cases.append({
                ...
                "assistant_messages": [],
                "model_responses": [],
                "status": "active"  # Track if this test case is still valid
            })

    # Process each round
    for round_idx in range(num_rounds):
        # Generate prompts for active test cases
        active_test_cases = [tc for tc in test_cases if tc["status"] == "active"]
        if not active_test_cases:
            print("Warning: No active test cases remaining")
            break

        prompts = []
        for test_case in active_test_cases:
            if round_idx == 0:
                prompt = f"""You will be interacting with one of two language models. 
                ...
                prompt = f"""Based on the hypothesis and previous interactions:

                Previous query: {test_case['assistant_messages'][-1]}
                Model's response: {test_case['model_responses'][-1]}

                Write your next query to help determine which model you're talking to:"""
            prompts.append(prompt)

        # Get queries from assistant
        try:
            request_info = {
                "pipeline_stage": "discriminative_validation",
                "round": str(round_idx),
                "batch": "initial_queries" if round_idx == 0 else "follow_up_queries"
            }
            queries = parallel_make_api_requests(
                prompts,
                api_provider,
                api_model_str,
                auth_key,
            ...
            # Store queries
            for test_case, query in zip(active_test_cases, queries):
                test_case['assistant_messages'].append(query)
        except Exception as e:
            print(f"Error in API requests for round {round_idx}: {e}")
            # Mark affected test cases as failed
            for test_case in active_test_cases:
                test_case["status"] = "failed"
            continue

        # Get model responses for each model
        for model in [starting_model, comparison_model]:
            model_test_cases = [tc for tc in active_test_cases if tc['target_model'] == model]
            if not model_test_cases:
                continue

            model_queries = [tc['assistant_messages'][-1] for tc in model_test_cases]
            
            try:
                all_responses = generate_responses_in_batches(
                    model=model,
                ...
                # Update test cases with responses
                for test_case, response in zip(model_test_cases, all_responses):
                    test_case['model_responses'].append(response)

            except Exception as e:
                print(f"Error generating responses for model batch: {e}")
                for test_case in model_test_cases:
                    test_case["status"] = "failed"

    # Get final predictions for successful test cases
    successful_test_cases = [tc for tc in test_cases if tc["status"] == "active"]
    if successful_test_cases:
        final_prompts = []
        for test_case in successful_test_cases:
            interaction_history = ' '.join([
                f'Query {i+1}: {q}\nResponse {i+1}: {r}\n' 
                for i, (q, r) in enumerate(zip(
                    test_case['assistant_messages'], 
                    test_case['model_responses']
                ))
            ])

            explain_reasoning_or_not = "First, explain your reasoning about which model you were talking to.\nThen on a new line, write your final prediction as just the number 1 or 2." if explain_reasoning else "Write your final prediction as just the number 1 or 2. Say nothing else, just the number corresponding to the model you were talking to."
            
            prompt = f"""Based on the hypothesis:
            "{test_case['description']}"

            And your interaction history:
            {interaction_history}

            {explain_reasoning_or_not}"""
        ...
        try:
            request_info = {
                "pipeline_stage": "discriminative_validation",
                "round": "final",
                "batch": "final_predictions"
            }
            final_responses = parallel_make_api_requests(
                final_prompts,
                api_provider,
                api_model_str,
                auth_key,
            ...
            for test_case, final_response in zip(successful_test_cases, final_responses):
                try:
                    if explain_reasoning:
                        parts = final_response.strip().split('\n')
                        prediction = int(parts[-1].strip())
                        reasoning = '\n'.join(parts[:-1])
                        all_reasonings[test_case['desc_idx']].append(reasoning)
                    else:
                        prediction = int(final_response.strip())

                    accurate = 1.0 if prediction == test_case['target_label'] else 0.0
                    all_predictions_accurate[test_case['desc_idx']].append(accurate)
                except (ValueError, IndexError) as e:
                    print(f"Error processing final response for test case {test_case['desc_idx']}: {e}")
                    all_predictions_accurate[test_case['desc_idx']].append(0.0)
                    if explain_reasoning:
                        all_reasonings[test_case['desc_idx']].append("Error processing response")

        except Exception as e:
            print(f"Error in final API requests: {e}")
            # Handle failed final predictions
            for test_case in successful_test_cases:
                all_predictions_accurate[test_case['desc_idx']].append(0.0)
                if explain_reasoning:
                    all_reasonings[test_case['desc_idx']].append("Failed to get final prediction")

    # Calculate final metrics
    hypothesis_accuracies = []
    hypothesis_p_values = []
    for predictions in all_predictions_accurate:
        if predictions:  # Only process if we have predictions

...
def decode(
    ...
    with torch.no_grad():
        ...
        if return_divergences:
            ...
            div_output = model.calculate_current_divergence(
                text_ids,
                metric=divergence_fnct,
                batch_size=8,
                end_tokens_to_only_consider=(
                    0 if include_prefix_in_divergences else generation_length
                ),
            ...
            if return_perplexities:
                ...
                comparison_model_perplexities = div_output[
                    "comparison_model_perplexities"
                ]
            if return_all_token_divergences:
                all_token_divergences = torch.tensor(
                    div_output["all_token_divergences"]
                )
                ...
                if print_texts:
                    ...
                    max_divergence = torch.max(
                        all_token_divergences[:, set_prefix_len:]
                    ).item()
                    min_divergence = torch.min(
                        all_token_divergences[:, set_prefix_len:]
                    ).item()
                    all_token_colorings = (
                        all_token_divergences - min_divergence
                    ) / max(max_divergence - min_divergence, 1e-7)

            text_ids = torch.cat(
                (
                    text_ids[:, :set_prefix_len],
                    torch.full((text_ids.size(0), 1), marker_id, device=device),
                    text_ids[:, set_prefix_len:],
                ),
                dim=1,
            )
            ...
            generated_tokens = [
                [tokenizer.convert_ids_to_tokens(t) for t in g] for g in generations
            ]
            ...
            if (
                beam_search_sort is None
                and not sort_by_divergences
                and not num_beams is None
            ):
            ...
            if sort_by_divergences:
                if single_prefix is None:
                    ...
                    block_avg_divergences = (
                        block_avg_divergences + 1e-7 * divergences.to(torch.float64)
                    )
                ...
                for i in range(0, len(original_indices), generations_per_prefix):
                    ...
                    sorted_block_indices = torch.tensor(
                        [x for _, x in sorted(zip(block_texts, block_indices))]
                    )
                    original_indices[i : i + generations_per_prefix] = (
            ...
            if return_perplexities:
                starting_model_perplexities = [
                    starting_model_perplexities[i] for i in indices
                ]
                comparison_model_perplexities = [
                    comparison_model_perplexities[i] for i in indices
                ]
            if return_all_token_divergences:
                ...
                for i in range(len(generated_tokens)):
                    if return_all_token_divergences:
                        ...
                        prompt_text = tokenizer.decode(
                            generations[i][: set_prefix_len + 1]
                        )
                        colored_text = ""
                        ...
                        printstr += (
                            str(round(divergences[i].item(), 5))
                            + ", "
                            + generated_texts[i]
                        )
        ...
        if not save_texts_loc is None:
            ...
            if return_perplexities:
                ...
                cols = cols + [
                    "starting_model_perplexity",
                    "comparison_model_perplexity",
                ]
            if return_all_token_divergences:
                df_list = [
                    item + [per_token_divs]
                    for item, per_token_divs in zip(
                        df_list, all_token_divergences.tolist()
                    )
                ]
